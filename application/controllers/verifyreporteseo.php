...
class Verifyreporteseo extends CI_Controller {
 ...
 function __construct()
 {
  ...
  {  
    if($this->session->userdata('logged_in'))
    {
      ...
      if($this->form_validation->run() == FALSE)
      {
        ...
        $this->load->view('reportes_eolico_view', $data);
      } 
  ...
  public function pagination() {
    ...
    //Go to private area          
    $config = array();
    $config["base_url"] = base_url() . "index.php/verifyreporteseo/pagination";
    $config["total_rows"] = $this->reportes->consultarNumDatos_eo($inicio, $fin);
    $config["per_page"] = 20;
    ...
    $data = array('nombre'=> $session_data['nombre']);
    $data["results"] = $this->reportes->consultar_datos_eo($inicio, $fin,$config["per_page"], $page);
    $data["links"] = $this->pagination->create_links();

    $this->load->view("reportes_eoconsulta_view", $data);
  }
  //////////////////////////////////////////////////////////////
  ...
  {    
    ...
    $delimiter = ",";
    $newline = "\r\n";
    $query = $this->reportes->get_datosconsulta_eo($inicio, $fin);    
